 ...
 document.addEventListener('scroll', () => {
    ...
    for (let i = 0; i < pageSections.length; i++) {
       ...
       if (scrollTracker['y'] <= 150) {
          pageSections[i].setAttribute('class', 'active');
       }
       ...
       item.addEventListener('click', event => {
          for (let i = 1; i <= pageSections.length; i++) {
             ...
             if (event.srcElement.id === `navItem_${i}`) {
                ...
                let navTest = document.querySelectorAll('.menu__link');
                for (let j = 0; j < pageSections.length; j++) {
                   navTest[j].classList.remove("active");
                   navTest[i - 1].classList.add("active");
                }
             }
          }
       })
